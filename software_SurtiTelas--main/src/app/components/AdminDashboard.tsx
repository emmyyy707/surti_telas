...
import logoBlanco from '@/assets/images/logos/surtitelas-logo.jpg';
...
case 'escaner':
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 overflow-x-hidden">
      ...
      {/* Sidebar */}
      <aside
        className={`bg-[var(--sidebar-bg)] text-white transition-all duration-300 ease-in-out flex flex-col ${
          isMobile
            ? `fixed inset-y-0 left-0 z-50 w-72 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`
            : `relative h-full ${sidebarCollapsed ? 'w-20' : 'w-72'}`
        }`}
      >
        {/* Header con logo y toggle */}
        <div className="flex items-center justify-between p-4 border-b border-[var(--sidebar-border)] relative">
...
className={`text-[var(--sidebar-text)] hover:text-white hover:bg-white/10 p-2 rounded-lg transition-all z-10 flex-shrink-0 ${
                           sidebarCollapsed ? 'absolute -right-3 top-1/2 -translate-y-1/2 bg-[var(--sidebar-bg)] border border-[var(--sidebar-border)] shadow-lg' : ''
                         }`}
            >
              {sidebarCollapsed ? (
          ...
          {isMobile && (
            <button
              onClick={() => setMobileMenuOpen(false)}
              className="text-[var(--sidebar-text)] hover:text-white hover:bg-[var(--sidebar-hover-bg)] p-2 rounded-lg transition-all"
            >
              <X className="h-5 w-5" />
            </button>
          )}
...
className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors ${
        ...
        <div className="p-4 border-t border-[var(--sidebar-border)]">
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-[var(--sidebar-text)] hover:bg-red-500/10 hover:text-red-500 ${
              sidebarCollapsed && !isMobile ? 'justify-center' : ''
            }`}
