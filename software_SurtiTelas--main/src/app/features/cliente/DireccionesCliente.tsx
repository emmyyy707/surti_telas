...
export function DireccionesCliente() {
  ...
  const handleDeleteDireccion = (id: string) => {
    if (window.confirm('óEstós seguro de eliminar esta dirección?')) {
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-[var(--text-primary)]">Direcciones</h1>
          <p className="text-[var(--text-secondary)] mt-1">Gestiona tus direcciones de enváo</p>
        </div>
        <Button className="bg-[var(--text-primary)] hover:bg-gray-800" onClick={() => handleOpenModal()}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {direcciones.map((direccion) => (
          <Card
            key={direccion.id}
            className={`p-6 bg-[var(--bg-elevated)] rounded-xl shadow-sm hover:shadow-md transition-shadow ${
              direccion.predeterminada ? 'border-2 border-gray-900' : ''
            }`}
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-full bg-[var(--text-primary)] text-white flex items-center justify-center">
                  {getTipoIcon(direccion.tipo)}
                </div>
                <div>
                  <h3 className="font-semibold text-lg text-[var(--text-primary)]">{direccion.nombre}</h3>
                  {direccion.predeterminada && (
                    <Badge className="bg-[var(--text-primary)] text-white mt-1">
      ...
      {direcciones.length === 0 && (
        <Card className="p-12 bg-[var(--bg-elevated)] rounded-xl shadow-sm text-center">
          ...
          <p className="text-[var(--text-secondary)] font-medium">No tienes direcciones guardadas</p>
          <p className="text-sm text-[var(--text-tertiary)] mt-1 mb-4">Agrega una dirección para facilitar tus envéos</p>
          <Button className="bg-[var(--text-primary)] hover:bg-gray-800" onClick={() => handleOpenModal()}>
            <Plus className="h-4 w-4 mr-2" />
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingDireccion
                ? 'Actualiza los datos de tu dirección de enváo'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full h-9 px-3 rounded-md border border-gray-200 bg-[var(--bg-elevated)] text-sm"
              >
          ...
          <DialogFooter>
            ...
            <Button className="bg-[var(--text-primary)] hover:bg-gray-800" onClick={handleSaveDireccion}>
