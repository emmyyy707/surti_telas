...
function toUser(dto: BackendAuthUser & { createdAt?: string; pedidosRealizados?: number; permisos?: string[] }): Usuario {
  const estado = dto.estado ? (dto.estado === 'INACTIVO' ? 'Inactivo' : 'Activo') : 'Activo';
  return {
    id: dto.id,
    nombre: dto.nombre,
    email: dto.email,
