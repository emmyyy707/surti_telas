...
test.describe('Gestion de Cotizaciones - Flujo E2E', () => {
  ...
  test('flujo: lista -> detalle -> gestionar cotizacion -> guardar -> enviar -> cierre', async ({ page }) => {
    ...
    const editor = page.locator('[data-testid="quotation-editor"]').first();
    await expect(editor).toBeVisible({ timeout: 10000 });

    const conceptRows = page.locator('[data-testid="concept-row"]').first();
    const firstDescripcion = conceptRows.locator('input').first();
    if (await firstDescripcion.count() > 0) {
      const currentVal = await firstDescripcion.inputValue();
      if (currentVal !== 'Prenda de algodon') {
    ...
    const editorAfterSave = page.locator('[data-testid="quotation-editor"]').first();
