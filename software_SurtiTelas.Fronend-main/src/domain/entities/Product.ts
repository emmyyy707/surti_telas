...
export type ProductPublicationStatus = 'Publicado' | 'Borrador' | 'Oculto';
...
export class Product {
  ...
  static validate(data: ProductData): void {
    if (!data.ref.trim()) {
      throw new Error('El producto debe tener una referencia');
    }
    ...
    if (!data.tela.trim()) {
      throw new Error('El producto debe tener una tela definida');
    }
    ...
    if (!Array.isArray(data.imagenes)) {
      throw new Error('Las imágenes del producto deben ser un arreglo');
    }
  ...
  publish(fechaPublicacion = new Date().toISOString()): Product {
    ...
    return this.withChanges({
      publicado: true,
      fechaPublicacion: this.fechaPublicacion ?? fechaPublicacion,
      estado: 'Activo',
    });
  }
